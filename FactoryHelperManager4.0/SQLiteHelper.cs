...
namespace FactoryHelperManager
{
    public class SQLiteHelper
    {
        ...
        public static void ExecuteSqlTran(List<string> SQLStringList,List<SQLiteParameter[]> sQLiteParameters)
        {
            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                conn.Open();
                ...
                try
                {
                    if (SQLStringList.Count == sQLiteParameters.Count)
                    {
                        for (int n = 0; n < SQLStringList.Count; n++)
                        {
                            string strsql = SQLStringList[n].ToString();
                            PrepareCommand(cmd, conn, null, strsql, sQLiteParameters[n]);
                            cmd.ExecuteNonQuery();
                        }
                        tx.Commit();
                    }
        ...
        public static DataSet Query(string SQLString)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                {
                    //LogHelper.WriteMessage("SQL语句：" + ex);
        ...
        public static void SetPassword()
        {
            ...
            {
                connection.Open();
                connection.ChangePassword("huaxingedu.cn");
            }
        }
