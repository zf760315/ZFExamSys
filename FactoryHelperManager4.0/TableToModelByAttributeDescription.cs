...
{
    public class TableToModelByAttributeDescription
    {
        /// <summary>
        ...
        public static List<T> ToList<T>(DataTable dt, bool dateTimeToString) where T : class, new()
        {
            ...
            if (dt != null)
            {
                ...
                SetListByModelAttributeDescription<T>(list, infos, dt, dateTimeToString);
        ...
        private static void SetListByModelAttributeDescription<T>(List<T> list, List<PropertyInfo> infos, DataTable dt, bool dateTimeToString) where T : class, new()
        {
                    ...
                    foreach (Attribute att in p.GetCustomAttributes(true))
                    {
                        ...
                        if (dscript != null)
                        {
                            if (dr[dscript.Description] != DBNull.Value)
                            {
                                ...
                                if (dr[dscript.Description].GetType() == typeof(DateTime) && dateTimeToString)
                                {
                                    tempValue = dr[dscript.Description].ToString();
                                }
                                try
                                {
                                ...
                                {
                                    LogHelper.WriteMessage("datable转实体类错误：\r\n" + ex);
                                }
                            }
                        }
                    }
